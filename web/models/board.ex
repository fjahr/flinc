defmodule Flinc.Board do
  use Flinc.Web, :model

  alias __MODULE__
  alias Flinc.{Repo, Permalink, List, Comment, Card, UserBoard, User}
  ...
  def preload_all(query) do
    ...
    cards_query = from c in Card, order_by: c.position, where: is_nil(c.deleted_at), preload: [[comments: ^comments_query], :members]
...
defimpl Poison.Encoder, for: Flinc.Board do
  def encode(model, options) do
    model
    |> Map.take([:name, :lists, :user, :members])
    |> Map.put(:id, Flinc.Board.slug_id(model))
